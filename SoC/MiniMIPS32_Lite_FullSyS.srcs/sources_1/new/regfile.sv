...
module regfile(
    ...
    always @(posedge cpu_clk_50M) begin
        if (cpu_rst_n == `RST_ENABLE) begin
            regs[ 0] <= `ZERO_WORD;
            regs[ 1] <= 32'h10101010;     // 注意：寄存器1和2复位后应该就是0x00000000，此处赋予随机值仅为了验证R-型指令，无法对寄存器赋值的。后续添加I-型指令后可恢复为赋值为0的情况
            regs[ 2] <= 32'h01011111;
            regs[ 3] <= `ZERO_WORD;
            regs[ 4] <= `ZERO_WORD;
            regs[ 5] <= `ZERO_WORD;
    ...
    // 读端口1的读操作
    // ra1是读地址，wa是写地址，we是写使能。wd是要写的数据
    always @(*) begin
    ...
    // 读端口2的读操作
    // ra2是读地址，wa是写地址，we是写使能。wd是要写的数据
    always @(*) begin
        if (cpu_rst_n == `RST_ENABLE)
            rd2 <= `ZERO_WORD;
        else if (ra2 == `REG_NOP)
